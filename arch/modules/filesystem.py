...
def mount_filesystems() -> None:
    ...
    ui.print_step_info(f"Mounting Btrfs ROOT subvolume '@{user_config['btrfs_subvol_root']}' to {mnt_base}...")
    core.run_command([
        "mount", "-o", f"subvol=/{user_config['btrfs_subvol_root']},{btrfs_mount_opts}",
    ...
    ui.print_step_info(f"Mounting Btrfs HOME subvolume '@{user_config['btrfs_subvol_home']}' to {mnt_base / 'home'}...")
    core.run_command([
        "mount", "-o", f"subvol=/{user_config['btrfs_subvol_home']},{btrfs_mount_opts}",
        lv_root_path_str, str(mnt_base / "home")
    ], check=True)

    ui.print_step_info(f"Mounting Btrfs VAR subvolume '@{user_config['btrfs_subvol_var']}' to {mnt_base / 'var'}...")
    core.run_command([
        "mount", "-o", f"subvol=/{user_config['btrfs_subvol_var']},{btrfs_mount_opts}",
    ...
    if user_config.get('btrfs_subvol_snapshots'):
        snapshots_subvol_name = str(user_config['btrfs_subvol_snapshots'])
        snapshots_mount_point = mnt_base / snapshots_subvol_name.lstrip('@') # e.g. /mnt/.snapshots
        ...
        ui.print_step_info(f"Mounting Btrfs SNAPSHOTS subvolume '@{snapshots_subvol_name}' to {snapshots_mount_point}...")
        core.run_command([
            "mount", "-o", f"subvol=/{snapshots_subvol_name},{btrfs_mount_opts}",
...
def verify_mounts(no_verify_arg: bool) -> None:
    ...
    if user_config.get('btrfs_subvol_snapshots'):
        snapshots_subvol_name = str(user_config['btrfs_subvol_snapshots'])
        snapshots_mount_point_name = snapshots_subvol_name.lstrip('@')
        expected_mounts.append(
            {"target": f"{mnt_base_str}/{snapshots_mount_point_name}", "source_pattern": btrfs_base_device, "fstype": "btrfs", "options_substring": f"subvol=/{snapshots_subvol_name}"}
        )
    ...
    if cfg.get_dry_run_mode():
        ...
        for line_idx, line_content in enumerate(content.splitlines()):
            ...
            if len(parts) >= 6 and parts[1] == "/" and "btrfs" in parts[2]:
                ...
                expected_subvol_opt_str: str = f"subvol=/{user_config['btrfs_subvol_root']}"
        ...
        if not root_line_found_and_correct:
            ...
            ui.print_color(f"Expected subvolume for root: subvol=/{user_config['btrfs_subvol_root']}", ui.Colors.PEACH)
